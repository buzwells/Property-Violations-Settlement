...
# load the violations data - use get_violations_path() to mitigate Knitr working dir issues
source(get_violations_path("R/load_violations.R"))
violations %>%  glimpse()
...
by_title <- violations %>% 
  ...
  summarize(Violation.Count = n(), Mean.Days.Open = mean(Days.Open)) %>%
  ungroup() %>% 
  filter(Violation.Count > 2000) %>% 
  mutate(Percent.Violations = (Violation.Count / nrow(violations %>% filter(Ordinance.Chapter %in% c('48','56'))))*100.00) %>%
  arrange(desc(Violation.Count))
